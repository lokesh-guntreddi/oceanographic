...
import {
  Menu, Home, Database, BarChart3, FileText,
  Settings, User, LogOut, Languages, Bell,
  Sparkles, Shield, Zap, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export function Layout({
  children, user, currentPage, onNavigate, onLogout, language
}: LayoutProps) {
  ...
  const getNavigationItems = (): NavigationItem[] => {
    const coreItems: NavigationItem[] = [
      { id: user?.role as RouteName, label: "Dashboard", icon: Home },           // researcher/policymaker/conservationist/admin
    ...
    const adminItems: NavigationItem[] = user?.role === "admin"
      ? [
          ...
          { id: "adminDashboard", label: "User Management", icon: User, isSubItem: true },
          { id: "adminDashboard", label: "System Administration", icon: Settings, isSubItem: true },
          { id: "adminDashboard", label: "Dataset Approvals", icon: FileText, isSubItem: true },
          { id: "adminDashboard", label: "Activity Logs", icon: BarChart3, isSubItem: true },
        ]
      : [];

    const crimeDetection: NavigationItem[] =
      user?.role !== "researcher"
        ? [{ id: "marineCrimeDetection", label: "Marine Crime Detection", icon: Shield }]
        : [];

    const roleSpecific: Record<string, NavigationItem[]> = {
      researcher: [
        { id: "otolithViewer", label: "Otolith Lab", icon: BarChart3 },
        { id: "ednaLab", label: "eDNALab", icon: Database }
      ],
      policymaker: [
        { id: "policyTools", label: "Policy Tools", icon: Settings }
      ],
      conservationist: [
        { id: "conservationTools", label: "Conservation Tools", icon: FileText }
      ],
      admin: []
    };
